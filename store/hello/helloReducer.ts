...
> = {
  ...
  setClientInfo(state, action: actionType<{clientInfo: any, chatSessionId: string}>) {
    const chatSessionId = action.urlData?.chatSessionId || action.payload?.chatSessionId || '';
    if (chatSessionId) {
      state[chatSessionId] = {
        ...state[chatSessionId],
        clientInfo: { ...state[chatSessionId]?.clientInfo, ...(action.payload?.clientInfo || {}) },
      };
    }
  },
