...
(function () {
    ...
    class HelloChatbotEmbedManager {
        ...
        handleScriptMutations(mutation) {
            mutation.removedNodes.forEach(node => {
                if (node.tagName === 'SCRIPT' && node.src && node.src.includes('chat-package')) {
                    this.cleanupChatbot();
                }
            });
        }
        ...
        createIframeContainer() {
            ...
            this.parentContainer = document.getElementById(this.elements.chatbotIframeContainer);
        ...
        processChatbotDetails(data) {
            ...
            window.postMessage({ type: 'initializeChatbot', data: data.data }, '*');
            this.state.tempDataToSend = {
                ...this.state.tempDataToSend,
                initializeChatbot: data.data
            }
        ...
        showIconIfReady() {
            if (this.state.interfaceLoaded && (document.getElementById("chatbot-main-script") ? true : this.state.delayElapsed)) {
    ...
    function sendMessageToChatbot(messageObj) {
        const iframeComponent = document.getElementById(helloChatbotManager.elements.chatbotIframeContainer);
        if (iframeComponent) {
