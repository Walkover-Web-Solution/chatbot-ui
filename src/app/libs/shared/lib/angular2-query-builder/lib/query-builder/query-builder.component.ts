...
export class QueryBuilderComponent extends BaseComponent implements OnInit, OnChanges, ControlValueAccessor, Validator {
    ...
    public holdOperator = ['<=', '>=', '==', 'in_last'];
    // hold operator array for which show select drop down in second operator.
    protected secondaryOperator = ['in_last'];
    // hold value of meta key available in any operator key is field name and value is meta object.
        ...
        if (type === 'object') {
            this.fields = Object.keys(config.fields)
                .map((value) => {
                    ...
                    // if type of date field then we have to store meta object
                    if (field.type === 'date') {
                        field.operators.forEach((e) => {
                            if (e.meta) {
                                this.metaKeyValue = {
                                    ...this.metaKeyValue,
                                    [e.mathematical_symbol]: e.meta,
                                };
                            }
                        });
                    }
    ...
    getInputType(field: string, operator: string, suboperator?: string): string {
        ...
        switch (operator) {
            ...
            case 'in':
            case 'not in':
                return type === 'category' || type === 'boolean' ? 'multiselect' : 'text';
            default:
    ...
    changeOperator(operatorValue: any, rule: Rule, suboperatorValue?: string): void {
        let prevOperatorValue, prevSuboperatorValue;
        if (operatorValue !== null) {
            prevOperatorValue = rule.operator;
            rule.operator = operatorValue;
        }
        if (suboperatorValue !== null) {
            prevSuboperatorValue = rule.suboperator;
            rule.suboperator = suboperatorValue;
        }
        if (this.disabled) {
            return;
        }
        ...
        // Set rule value null only if the operator requires different type of input
        if (
            rule.value &&
            (rule.type.toLowerCase() === 'date'
                ? this.holdOperator.includes(prevOperatorValue) !== this.holdOperator.includes(rule.operator)
                : this.getInputType(rule.field, prevOperatorValue, prevSuboperatorValue) !==
                  this.getInputType(rule.field, rule.operator, rule.suboperator))
        ) {
            rule.value = null;
        }
        ...
        // here code for Date type of filed which has operators have meta key value.
        if (this.secondaryOperator.includes(rule.operator)) {
            rule.secondaryoperator = undefined;
        } else {
            delete rule.secondaryoperator;
        }
    ...
    changeField(fieldValue: any, rule: Rule): void {
        ...
        // Reset operator and value only if fields are of different type
        if (nextField.type !== oldFieldType) {
    ...
    protected minMaxValidatorForNumber(rule: any, min: number = null, max: number = null) {
        ...
        if (this.secondaryOperator.includes(rule.operator) && !rule.secondaryoperator) {
            rule.isValid = false;
            return;
        }
        this.handleTouched();
    ...
    protected secondaryOperatorChange(rule: Rule) {
        if (rule.value) {
            rule.isValid = true;
        }
        this.handleTouched();
