...
export interface IPlanAmountFormatted {
    ...
    plan_amount: number;
    plan_type: string;
    values: Array<{ amount: number; currency_code: string; dial_plan_id?: number; admin_limit: number }>;
}
...
export interface IPlanCreateReq {
    ...
    features: { id: number; is_visible: boolean }[];
...
export const CURRENCIES_WITH_IDS = {
    1: { symbol: '₹', name: 'INR' },
    2: { symbol: '$', name: 'USD' },
    3: { symbol: '£', name: 'GBP' },
};
...
export interface IProratedAmountResponse {
    currency: ICurrency;
    new_plan: IProratedAmount;
    old_plan: IProratedAmount;
}

export interface IProratedServiceAmountResponse {
    net_amount: IProratedAmountNetAmount;
}

export interface ICompanyPlanService {
    company_plan_id: number;
    free_credits: number;
    id: number;
    max_paid_credits: number;
    overusage_limit_admin: number;
    overusage_limit_user: number;
...
export interface IProratedAmountNetAmount {
    ...
    new_plan_charges: number;
    old_plan_charges: number;
}
...
export interface IBillingWhatsAppReportPayload {
    start_date: string;
    end_date: string;
    integrated_no?: string;
    waba_id?: number;
...
export interface IBillingWhatsAppReportResponse {
    _id: string;
    username: string;
    panel_user_id: number;
    currency: string;
    ...
    start_date: string;
    end_date: string;
    subscription_data: IBillingWhatsAppReportGeneralData;
    vendor_data: IBillingWhatsAppReportGeneralData;
    profit: IBillingWhatsAppReportGeneralData;
    updated_at: string;
    created_at: string;
}

export interface IBillingWhatsAppReportGeneralData {
    marketing: IBillingWhatsAppAmount;
    authentication: IBillingWhatsAppAmount;
    utility: IBillingWhatsAppAmount;
    service: IBillingWhatsAppAmount;
    total: IBillingWhatsAppAmount;
}

export interface IBillingWhatsAppAmount {
    sessions?: number;
    amount: number;
}
...
export interface ISalesReportResponse {
    ...
    lastSpanLength: number;
    lastSpanRevenue: number;
    lastThreeSpanData: any[];
    revenueChange: number;
    revenuePercentChange: number;
    spanBeforeLastLength: number;
    spanBeforeLastRevenue: number;
    userName: string;
    user_pid: string;
    acManager: string;
}
