...
export class DomainService {
    ...
    public addIps(request: AddIpRequest, smtpCreId: string): Observable<BaseResponse<any, AddIpRequest>> {
        return this.http.post<BaseResponse<any, AddIpRequest>>(
            DomainUrls.addIps(this.emailBaseUrl).replace(':smtpCreId', smtpCreId),
            request
        );
    }

    public removeIps(smtpCreId: string, ipId: string): Observable<BaseResponse<any, AddIpRequest>> {
        return this.http.delete<BaseResponse<any, AddIpRequest>>(
            DomainUrls.removeIps(this.emailBaseUrl).replace(':smtpCreId', smtpCreId).replace(':ipId', ipId)
        );
    }
}
