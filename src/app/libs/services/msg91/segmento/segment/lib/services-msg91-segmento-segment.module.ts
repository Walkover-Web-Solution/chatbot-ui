...
export class SegmentService {
    ...
    public getFormulaQuery(
        request: { query: string },
        phoneBookId: number
    ): Observable<BaseResponse<any, { query: string }>> {
        return this.http.post<BaseResponse<any, { query: string }>>(
            SegmentUrls.getFormulaQuery(this.segmentoBaseUrl).replace(':phoneBookId', phoneBookId.toString()),
            request
        );
    }
}
