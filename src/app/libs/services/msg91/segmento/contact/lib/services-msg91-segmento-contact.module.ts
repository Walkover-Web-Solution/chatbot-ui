...
import {
    ...
    IUploadFileReq,
    PhoneBookAutomationCountResModel,
    ExportResponseModel,
    LogTypesModel,
...
export class ContactsService {
    ...
    public deleteBulkContact(
        ...
        return this.http
            ...
            .pipe(
                map((res) => {
                    const data: BaseResponse<any, any> = res;
                    data.request = { from: from, contactId: request.objectIds };
                    return data;
                })
            );
    }
    ...
    public uploadContactsByFile(request: IUploadFileReq, phoneBookId: number): Observable<BaseResponse<any, any>> {
        ...
        return this.http.post<BaseResponse<IUploadContactsByFileRes, IUploadFileReq>>(
            ContactUrls.uploadContactsByFileUrl(this.segmentoMsg91BasePath + '/api').replace(
                ':phoneBookId',
    ...
    public getDateTimeFormats(): Observable<BaseResponse<IDateTimeFormatResponse, void>> {
        return this.http.get<BaseResponse<IDateTimeFormatResponse, void>>(
            ContactUrls.dateTimeFormats(this.segmentoV1BaseUrl)
        );
    }

    public getAllContactsData(params: any, phoneBookId: string): Observable<BaseResponse<IMobileContacts, void>> {
        return this.http.get<BaseResponse<IMobileContacts, void>>(
            ContactUrls.getMobileContact(this.segmentoV1BaseUrl).replace(':phonebookId', phoneBookId),
            params
        );
    }

    public getPhoneBookAutomationCount(
        phoneBookId: string
    ): Observable<BaseResponse<PhoneBookAutomationCountResModel, string>> {
        return this.http.get<BaseResponse<PhoneBookAutomationCountResModel, string>>(
            ContactUrls.getPhoneBookAutomationCount(this.baseUrl).replace(':phonebookId', phoneBookId)
        );
    }

    public getExportContactLogDetail(
        request: any,
        phoneBookId: string
    ): Observable<BaseResponse<IPaginatedResponse<ExportResponseModel[]>, null>> {
        return this.http.get<BaseResponse<IPaginatedResponse<ExportResponseModel[]>, null>>(
            ContactUrls.getExportLogUrl(this.segmentoV1BaseUrl).replace(':phoneBookId', phoneBookId),
            request
        );
    }

    public getExportLogTypes(): Observable<BaseResponse<LogTypesModel[], null>> {
        return this.http.get<BaseResponse<LogTypesModel[], null>>(ContactUrls.getExportLogTypesUrl(this.baseUrl));
    }
}
