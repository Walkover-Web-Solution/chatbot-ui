...
import {
    ...
    IConversionJourneyRes,
} from '@msg91/models/campaign-models';
import { omit } from 'lodash';
import { ICampaignFailedReqLog, ICampaignFailedResLog } from '@msg91/models/campaign-models';
...
export class CampaignService {
    ...
    public getCampaignRemainingLimit(slug: string): Observable<BaseResponse<any, any>> {
        return this.http.get<BaseResponse<any, any>>(
            CampaignUrls.getCampaignRemainingLimit(this.baseUrl).replace(':slug', slug)
        );
    }
    ...
    public getAnalytics(params: any, slug: string): Observable<any> {
        return this.http.get(CampaignUrls.getAnalytics(this.baseUrl).replace(':slug', slug), params);
    }
    ...
    public getServiceDetails(params: any): Observable<any> {
        const payload = omit(params, ['serviceName']);
        return this.http.post<any>(
            CampaignUrls.getServiceDetails(this.baseAnalyticsUrl).replace(':serviceName', params.serviceName),
            payload
        );
    }
    public exportAnalyticsLogs(params: any): Observable<any> {
        const payload = omit(params, ['serviceName']);
        return this.http.post<any>(
            CampaignUrls.exportAnalyticsLogs(this.baseAnalyticsUrl).replace(':serviceName', params.serviceName),
            payload
        );
    }
    public getFailedLogs(
        params: ICampaignFailedReqLog
    ): Observable<BaseResponse<IPaginatedResponse<ICampaignFailedResLog>, ICampaignFailedReqLog>> {
        return this.http.get<BaseResponse<IPaginatedResponse<ICampaignFailedResLog>, ICampaignFailedReqLog>>(
            CampaignUrls.getFailedLogs(this.baseUrl),
            params
        );
    }
    public getCampaignLogDetails(id: string): Observable<BaseResponse<any, string>> {
        return this.http.get<BaseResponse<any, string>>(
            CampaignUrls.getCampaignLogDetails(this.baseUrl).replace(':logId', id)
        );
    }

    public getConversionJourney(slug: string): Observable<BaseResponse<IConversionJourneyRes[], any>> {
        return this.http.get<BaseResponse<IConversionJourneyRes[], any>>(
            CampaignUrls.getConversionJourney(this.baseUrl).replace(':slug', slug)
        );
    }
}
