...
export class TelegramService {
    constructor(
        ...
        @Inject(ProxyBaseUrls.ReportsUrl) private reportUrl: any,
    ...
    public fetchTelegramLogs(request: any): Observable<ITelegramAdminLogResponse> {
        return this.http.get<ITelegramAdminLogResponse>(TelegramClientUrls.telegramLogs(this.reportUrl), request);
    }

    public exportTelegramLogs(request: any): Observable<any> {
        const convertToQueryParam = this.http.objectToParams({ ...request });
        const url = TelegramClientUrls.exportTelegramLogs(this.reportUrl).concat(`?${convertToQueryParam}`);
