...
export class MultiSelectComponent extends BaseComponent implements OnChanges, OnInit, OnDestroy, ControlValueAccessor {
    ...
    /** mat field appearance  */
    @Input() appearance: MatFormFieldAppearance = 'outline';
    /** If true, will make multi select list  */
    @Input() isMultiSelect: boolean = true;
    /** Required to check the checkboxes by default when component is rendered. This will
     * be set to false for update scenarios where no value is selected. If this is not
     * provided then even in update case the component will by default check all the boxes
     * if 'showSelectAll' is true
     */
    @Input() enableDefaultSelection: boolean = true;
    /** Enforces the selection, if true, will not allow no selection and by default will select all the values */
    @Input() enforceSelection: boolean;
    /** Store no record label value */
    @Input() multiSelectLabelForNoValue: string;
    ...
    public ngOnChanges(changes: SimpleChanges): void {
        if (
            ...
            if (this.showSelectAll) {
                ...
                if (this.multiSelectValues?.length && this.multiSelectValues?.[0]?.value) {
                    (this.multiSelectValues as Array<MultiSelectGroupOptions>).forEach((value) => {
                        this.allGroupedValues.push(...value.value);
                    });
                } else {
                    this.allGroupedValues = this.multiSelectValues?.length ? this.multiSelectValues : [];
                }
                if (this.enableDefaultSelection) {
        ...
        this.multiSelectControl.valueChanges
            ...
            .subscribe(() => {
                if (!isEqual(this.previousSelectedValues, this.multiSelectControl.value)) {
                    ...
                    if (!this.emitSelectedValueOnMenuClose) {
                        if (!this.multiSelectControl.value?.length && this.enforceSelection) {
    ...
    public compareConditionally = (firstValue: any, secondValue: any): boolean => {
        if (
            this.multiSelectValuesId &&
            !firstValue[this.multiSelectValuesId] &&
            !secondValue[this.multiSelectValuesId]
        ) {
            return false;
        }
    ...
    private setControlValue(val: Array<MultiSelectGroupOptions> | Array<any>, shouldEmit: boolean = true): void {
        ...
        if (!this.shouldGroupOptions) {
            ...
            let selectedGroupedValues = [];
            if (currentValue?.length && currentValue?.[0]?.value) {
                (currentValue as Array<MultiSelectGroupOptions>).forEach((value) => {
                    selectedGroupedValues.push(...value.value);
                });
            } else {
                selectedGroupedValues = currentValue?.length ? currentValue : [];
            }
            this.multiSelectControl.setValue(selectedGroupedValues);
        }
        if (shouldEmit) {
            this.onChange(this.multiSelectControl.value);
        }
