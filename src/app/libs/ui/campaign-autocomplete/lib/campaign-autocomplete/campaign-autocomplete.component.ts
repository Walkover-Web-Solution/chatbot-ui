import { DEBOUNCE_TIME, convertChannelTypeResponse } from '@msg91/constant';
...
export class CampaignAutocompleteComponent extends BaseComponent implements OnInit, AfterViewInit, OnChanges {
    @Input() campaignForm: FormControl;
    ...
    @Input() operationAllowed: boolean;
    @Input() hideCreateCampaignOption: boolean = true;
    @Input() hideDuplicateCampaignOption: boolean = true;
     ...
     * @default outline
     */
    @Input() formFieldAppearance = 'outline';
    /**
        ...
        if (this.createCampaignURL) {
            ...
            this.router.navigate(
                [
                    ...
                    {
                        outlets: {
                            primary: ['campaigns', 'flow'],
                            navigationRouter: ['campaigns'],
                        },
                    },
                ],
                {
    ...
    public getCampaignChannel() {
        this.flowService.getCampaignChannelType().subscribe({
            next: (channel) => {
                const convertedData = convertChannelTypeResponse(channel?.data, this.environment.prodServer);
                if (!channel?.hasError && convertedData?.length) {
                    this.allChannel$ = of(convertedData.filter((f) => f.name !== 'Condition'));
                }
            },
        });
    }
