...
async function getCSSRules(styleSheets: CSSStyleSheet[], options: Options): Promise<CSSStyleRule[]> {
    ...
    // for css constructor name please follow https://drafts.csswg.org/cssom/#concept-css-rule-type.
    styleSheets.forEach((sheet) => {
        if ('cssRules' in sheet) {
            const isNotSameOriginSheet = sheet.href && !sheet.href.startsWith(location.origin);
            if (isNotSameOriginSheet) {
                if (!sheet.href.includes('stripo')) {
