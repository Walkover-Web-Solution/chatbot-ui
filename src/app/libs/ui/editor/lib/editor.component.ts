...
export class EditorComponent extends BaseComponent implements OnInit, OnChanges, OnDestroy {
    ...
    @Input() editorHeight: string = null;
    @Input() autoIncreaseHeight: boolean = false;
    /** Show note above app editor */
    @Input() public noteText: string = '';
     ...
     */
    public jsCompletionProvider: monaco.IDisposable;
    public textCompletionProvider: monaco.IDisposable;
    public jsonCompletionProvider: monaco.IDisposable;
    ...
    public editorInit(editor: MonacoStandaloneCodeEditor): void {
        ...
        if (
            ...
            this.jsCompletionProvider?.dispose();
            this.textCompletionProvider?.dispose();
            this.jsonCompletionProvider?.dispose();
    ...
    public ngOnDestroy(): void {
        ...
        if (typeof this.jsCompletionProvider?.dispose === 'function') {
            this.jsCompletionProvider.dispose();
        }
        if (typeof this.textCompletionProvider?.dispose === 'function') {
            this.textCompletionProvider.dispose();
        }
        if (typeof this.jsonCompletionProvider?.dispose === 'function') {
            this.jsonCompletionProvider.dispose();
        }
    ...
    public assignToVariable(item: itemType): void {
        ...
        if (findPhoneBook?.label) {
            const splitted = (item.label as string).split(' -> ');
    ...
    public showSuggestions(): void {
        this.jsCompletionProvider = monaco.languages.registerCompletionItemProvider('javascript', {
                    ...
                    if (label && wordUntilPosition.word.trim() === '$') {
                        ...
                        if (findExisting?.length) {
                            dataByWord = this.completionItemWithSignKey[wordUntilPosition.word.trim()].filter(
                                ...
                                (phoneBook: any) => phoneBook.label.split(' -> ')[0] === label
                            );
                        } else {
        ...
        this.textCompletionProvider = monaco.languages.registerCompletionItemProvider('plaintext', {
            triggerCharacters: Object.keys(this.completionItemWithSignKey),
            provideCompletionItems: (model, position) => {
                const wordUntilPosition = model.getWordUntilPosition(position);
                if (this.completionItemWithSignKey[wordUntilPosition.word.trim()]) {
        ...
        this.jsonCompletionProvider = monaco.languages.registerCompletionItemProvider('json', {
            triggerCharacters: Object.keys(this.completionItemWithSignKey),
            provideCompletionItems: (model, position) => {
                const wordUntilPosition = model.getWordUntilPosition(position);
                if (this.completionItemWithSignKey[wordUntilPosition.word.trim()]) {
