...
export class MentionDirective implements OnChanges, OnDestroy {
    ...
    @Input() mentionDropDownStyle: any;

    @Input() hideListIfEmpty: boolean = false;

    @Input() closeList: boolean = false;

    @Input() enableInteractiveMessage: boolean;
            ...
            // ignore shift when pressed alone, but not when used with another key
                ...
                if (!this.activeConfig?.allowSpace && event.keyCode === KEY_SPACE) {
                    ...
                    if (event.keyCode === KEY_TAB || event.keyCode === KEY_ENTER) {
                        ...
                        if (!this.enableInteractiveMessage || typeof text === 'string') {
    ...
    showSearchList(nativeElement: HTMLInputElement) {
        ...
        this.searchList.showLargeBox = this.mentionConfig.showLargeBox;
        this.searchList.activeIndex = 0;

        // used for opening dropdown at caret position
        let positionSpan = document.createElement('input');
        let textAreaPosition = this._element.nativeElement.getBoundingClientRect();
        let caretPosition = this.returnCaretPosition();
        positionSpan.setAttribute('id', 'itemForIndex');
        positionSpan.style.position = 'fixed';
        positionSpan.style.left = textAreaPosition?.left + caretPosition.x + 'px';
        document.getElementsByTagName('body')[0].appendChild(positionSpan);

        this.searchList.position(positionSpan, this.iframe);
        document.body.removeChild(positionSpan);

        // since there were multiple occurences of mention list, it was applying top and left position on wrong mention list element, so we applied left and top on current mention list element
        setTimeout(() => {
            // Array.from(document.getElementsByClassName('mention-list') as HTMLCollectionOf<HTMLElement>)[0].style.top =
            //     (this.mentionDropDownStyle?.height ?? textAreaPosition?.top + caretPosition.y + 10) + 'px';
            this._element.nativeElement.nextSibling.style.top =
                (this.mentionDropDownStyle?.height ?? textAreaPosition?.top + caretPosition.y + 10) + 'px';

            if (
                this.mentionDropDownStyle?.maxFromLeft &&
                window.innerWidth - (textAreaPosition?.left + caretPosition.x) < this.mentionDropDownStyle?.maxFromLeft
            ) {
                // Array.from(
                //     document.getElementsByClassName('mention-list') as HTMLCollectionOf<HTMLElement>
                // )[0].style.left = window.innerWidth - this.mentionDropDownStyle?.maxFromLeft + 'px';

                this._element.nativeElement.nextSibling.style.left =
                    window.innerWidth - this.mentionDropDownStyle?.maxFromLeft + 'px';
            }
        }, 0);
        // this.searchList.position(nativeElement, this.iframe); // open dropdown at textarea position
        window.requestAnimationFrame(() => this.searchList.reset());
    }

    private returnCaretPosition(): any {
        let textArea = this._element.nativeElement;
        let start = textArea.selectionStart;
        let end = textArea.selectionEnd;
        let copy = this.createTextAreaCopy(textArea);
    ...
    private createTextAreaCopy(textArea): any {
        let copy = document.createElement('div');
        copy.textContent = textArea.value?.length ? textArea.value : '#';
