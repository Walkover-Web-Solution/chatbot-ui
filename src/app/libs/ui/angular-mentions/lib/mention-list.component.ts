...
export class MentionListComponent implements AfterContentChecked {
    ...
    private positionElement(
        ...
        const xPosition = Number(this.list.nativeElement.clientWidth) + Number(left);

        if (xPosition >= screen.width) {
            left = screen.width - Number(this.list.nativeElement.clientWidth) - 50;
        }
    ...
    private getBlockCursorDimensions(nativeParentElement: HTMLInputElement) {
        ...
        return {
            height: !isNaN(parseFloat(parentStyles.lineHeight)) ? parseFloat(parentStyles.lineHeight) : 0,
            width: !isNaN(parseFloat(parentStyles.fontSize)) ? parseFloat(parentStyles.fontSize) : 0,
        };
    }
}
