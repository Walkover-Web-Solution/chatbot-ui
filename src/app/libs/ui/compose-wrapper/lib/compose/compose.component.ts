...
export class ComposeWrapperComponent extends BaseComponent implements OnInit, OnDestroy, OnChanges {
    ...
    @Input() public deleteConfirmationMessage: string = `Are you sure you want to discard?`;
    ...
    // we are doing this because we need forms, so we check pristine value in it on close dialog
    @Input() forms: any;
    ...
    public onNoClick(): void {
        if (this.forms?.length) {
            const isUserChangedValue = this.checkIfFormIsDirty(this.forms);
        ...
        componentInstance.confirmationMessage = this.deleteConfirmationMessage;
