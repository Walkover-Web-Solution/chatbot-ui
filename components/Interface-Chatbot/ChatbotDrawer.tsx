...
const ChatbotDrawer = ({
  ...
  const handleVoiceCall = async () => {
    ...
    let overrideChatId;
    if (!currentChannelId && Array.isArray(channelList) && channelList.length > 0) {
    ...
    const data = await sendMessageToHello('', '', true, overrideChannelId || currentChannelId, overrideChatId || currentChatId);
