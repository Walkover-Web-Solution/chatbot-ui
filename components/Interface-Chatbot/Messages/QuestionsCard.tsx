...
import { ChevronRight, SkipForward, Check, HelpCircle, Lock } from "lucide-react";
...
interface QuestionsCardProps {
    ...
    isLoading?: boolean;
    // When true the card is frozen — inputs disabled, action buttons hidden, and a
    // "moved on" notice is shown. Used for historical / non-latest messages.
    disabled?: boolean;
}

export default function QuestionsCard({ questions, onAnswersSubmit, isLoading = false, disabled = false }: QuestionsCardProps) {
    ...
    }, [disabled, isRequired, isLastQuestion, answers, onAnswersSubmit]);

    if (!currentQuestion) return null;

    return (
        <div className={`mb-4 w-full not-prose ${disabled ? "opacity-70" : ""}`} aria-disabled={disabled}>
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-md overflow-hidden">
                {disabled && (
                    <div className="px-5 py-2 bg-slate-100 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700 flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
                        <Lock className="w-3.5 h-3.5" />
                        <span>This question session is closed — you continued the conversation.</span>
                    </div>
                )}
                {/* Header */}
                ...
                <div className="px-5 py-6 space-y-4">
                                    ...
                                    return (
                                        <label
                                            ...
                                            className={`flex items-center gap-2 p-2.5 rounded-lg border transition-all ${
                                                disabled
                                                    ? "cursor-not-allowed border-slate-200 dark:border-slate-700"
                                                    : "cursor-pointer"
                                            } ${
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea
                            ...
                            disabled={disabled}
                            className="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none disabled:cursor-not-allowed disabled:opacity-60"
                            rows={3}
                        />
                    </div>
                </div>

                {/* Action Buttons — hidden when the card is frozen */}
                {!disabled && (
